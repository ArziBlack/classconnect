...
import { IAPIResponse, INotification, IProfileImage } from "../../typings/student";
...
export const updateTutorProfileImage = createAsyncThunk<IAPIResponse, { pImage: IProfileImage }, { rejectValue: string }>("tutor/update-profile-image", async ({ pImage }, thunkAPI) => {
    try {
      const token = sessionStorage.getItem("token");
      const params = {
        headers: {
